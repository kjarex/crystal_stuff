...
class CubeSQLException < Database::Exception
end

class CubeSQL < Database::Adapter
  ...
  def initialize (uri : URI)
    p uri
    initialize uri.host.¬, uri.user.¬, uri.password.¬, uri.port,  encryption: Encryption::SSL
    use uri.path[1..-1]
  end

  def initialize (host : String, user : String, pass : String, port : Int32?= nil, timeOut : Int32= DEFAULT_TIMEOUT, encryption : Encryption= Encryption::None, sslCertificatePath : String= "")
    port||= DEFAULT_PORT
  ...
  def tables
    self.select("SHOW TABLES;").to_h.map{|x| x.values[0]} #TODO don't go via hash for no reason
  end
end
...
class CubeSQLResult
  ...
  def to_h
    (1...getNumRows).map do |row|
      row_ = Hash(String, String | Int64 | Int32 | Float64 | Float32 | Bytes | Nil).new
...
Database.register "cubesql", CubeSQL
